...
# Travail à faire:
# Partie 1
1. À partir des formules et explications ci-haut, compléter le fichier Python, là où vous voyez une lettre entre parenthèses suivi de la mention À COMPLÉTER en commentaire. 

2. Ensuite, programmer la résolution d'équation de Kepler (5) ci-haut avec la méthode de Newton vue en classe. La variable dans l'équation de Kepler est l'anomalie excentrique $E$. Les 2 autres valeurs sont des constantes. Voici les étapes à suivre:
...
- Si la valeur absolue du résultat de l'équation de Kepler quand on y remplace E par sa valeur initiale est plus petite que  $1\times10^-12$, retournez E directement. 
...
- un module avec l'interface (voir partie 3). 
- un module main() qui démarre le tout
...
Interface:
- Faire une interface qui utilise une *image cliquable* pour faire un choix d'animation. Les images choisies peuvent être n'importe lesquelles, pourvu qu'il soit écrit quelque part quelle animation est sélectionnée. 

- Vous devez trouver une manière de faire sélectionner les valeurs de
...
- La gestions des erreurs doit être intégrée. 
  - Au moins un try..except pertinent dans tout le code
  - Au moins un popup (réussite ou erreur ou avertissement)
- Images cliquables obligatoires
- Les modules doivent être respectés et connectés entre eux
