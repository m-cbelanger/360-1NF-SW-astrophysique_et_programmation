> Note importante: Lors de l'atelier présenté par Thomas Vandal, vous n'aurez qu'à être présents et effectuer les commandes avec lui pour réussir à avoir tous vos points pour l'évaluation. Toutefois, je vais retirer 0.5/2 à toute personne qui se présente le 27 avril sans avoir installé les requis et fait les étapes préalables***.
...
***Pour l'atelier de Thomas Vandal, on installera les éléments de la liste du fichier texte requirements. Vous pouvez les installer un par un ou bien télécharger le fichier requirements.txt, le placer dans un dossier, le même que celui affiché quand vous ouvrez Outils/Ouvrir la console du système. Par exemple, dans mon ordinateur, c'est dans le bureau de mon OneDrive:
![](img/chemin.png) 

Une fois le fichier téléchargé, on fait:
```
...
# Trouver des images en ligne (pour s'amuser!)
