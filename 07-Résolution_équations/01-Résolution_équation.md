...
La méthode de Newton a été optimisée avec fsolve de la librairie scipy.optimize. Par contre, dans cette partie de cours, nous allons apprendre à décortiquer un problème et suivre un algorithme. Pas de raccourci!

Voici le principe de la méthode numérique qui permet de trouver les racines d'une fonction continue et dérivable avec un point de départ choisit à tâtons, près d'un zéro. L'idée est la suivante:
...
2. Préparer la fonction

   - Définir le symbole x avec sympy
   - Écrire la fonction f(x) (ex: x**2 - 3*x - 10)

3. Calculer la dérivée

   - Utiliser sp.diff() pour obtenir f'(x) automatiquement
   - Créer une fonction df(valeur) qui évalue f'(x) en un point numérique avec .subs() et .evalf()

4. Choisir un point de départ

   - Fixer une valeur initiale x0 proche de la racine cherchée

5. Itérer avec le critère d'arrêt

   - Fixer une precision (ex: 1e-14)
   - Initialiser difference = 1 (pour entrer dans la boucle)
   - Tant que difference > precision :

   - Sauvegarder xn dans temporaire
   - Calculer le prochain xn avec la formule : 

    $x_{n+1}=x_n−\dfrac{f(x_n)}{f'(x_n)}$

   - Mettre à jour la différence = |temporaire - xn|



6. Afficher le résultat

   - Afficher la racine approximative xn
